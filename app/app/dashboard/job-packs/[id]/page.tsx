    ...
    <div className="">
      ...
      <nav className="container mx-auto px-4 py-6 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          <Link href="/dashboard/job-packs" className="text-[#607086] hover:text-[#1A237E]">
            ← Back to Job Packs
          </Link>
        </div>
      </nav>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="bg-white/60 backdrop-blur-lg rounded-2xl p-6 border border-black/[0.06] mb-6">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div>
                <h1 className="text-3xl font-bold text-[#1A237E]">{jobPack.company}</h1>
                <p className="text-xl text-[#607086]/80">{jobPack.job_title}</p>
              </div>
              <div className="flex items-center gap-4">
                <div className="text-center">
                  ...
                  <div className="text-[#607086]/80 text-sm">ATS Score</div>
                </div>
                {/* Export Dropdown */}
                <div className="relative group">
                  <button
                    className="bg-blue-600 hover:bg-blue-700 text-[#1A237E] px-4 py-2 rounded-lg transition-colors"
                    data-testid="export-btn"
                  >
                    Export ▼
                  </button>
                  <div className="absolute right-0 mt-2 w-40 bg-white rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10">
                    <button
                      onClick={() => handleExport("pdf")}
                      className="w-full px-4 py-2 text-left text-[#1A237E] hover:bg-slate-700 rounded-t-lg"
                    >
                      Download PDF
                    </button>
                    <button
                      onClick={() => handleExport("docx")}
                      className="w-full px-4 py-2 text-left text-[#1A237E] hover:bg-slate-700 rounded-b-lg"
                    >
                      Download DOCX
                    </button>
                  </div>
                ...
                <button
                  ...
                  disabled={deleting}
                  className="bg-red-600 hover:bg-red-700 disabled:bg-red-800 text-[#1A237E] px-4 py-2 rounded-lg transition-colors"
          ...
          <div className="bg-white/60 backdrop-blur-lg rounded-2xl p-6 border border-black/[0.06]">
            ...
            {activeTab === "overview" && (
              <div className="space-y-6">
                {/* ATS Score Summary */}
                <div>
                  <h2 className="text-xl font-bold text-[#1A237E] mb-4">ATS Compatibility</h2>
                ...
                {jobPack.cultural_fit_warnings && jobPack.cultural_fit_warnings.length > 0 && (
                  <div>
                    <h2 className="text-xl font-bold text-[#1A237E] mb-4">⚠️ Cultural Fit Warnings</h2>
                ...
                {/* Quick Stats */}
                <div>
                  <h2 className="text-xl font-bold text-[#1A237E] mb-4">Package Contents</h2>
                  <div className="grid md:grid-cols-3 gap-4">
                    <div className="bg-white/40 rounded-lg p-4">
                      <div className="text-3xl mb-2">📝</div>
                      <h3 className="text-[#1A237E] font-semibold">Tailored Resume</h3>
                      <p className="text-[#607086]/80 text-sm">Optimized for this role</p>
                    </div>
                    <div className="bg-white/40 rounded-lg p-4">
                      <div className="text-3xl mb-2">✉️</div>
                      <h3 className="text-[#1A237E] font-semibold">Cover Letter</h3>
                      <p className="text-[#607086]/80 text-sm">Personalized for {jobPack.company}</p>
                    </div>
                    <div className="bg-white/40 rounded-lg p-4">
                      <div className="text-3xl mb-2">📊</div>
                      <h3 className="text-[#1A237E] font-semibold">ATS Analysis</h3>
                      <p className="text-[#607086]/80 text-sm">Keywords & recommendations</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {activeTab === "resume" && (
              <div>
                <div className="flex justify-between items-center mb-4">
                  <h2 className="text-xl font-bold text-[#1A237E]">Tailored Resume</h2>
                  <button
                    onClick={fetchDiff}
                    disabled={loadingDiff}
                    className="bg-white/60 hover:bg-white/20 text-[#1A237E] px-4 py-2 rounded-lg transition-colors"
                ...
                <div className="bg-white/40 rounded-lg p-6">
                  ...
                  <pre className="text-[#607086] whitespace-pre-wrap font-sans text-sm leading-relaxed">
            ...
            {activeTab === "ats" && atsScan && (
              <div className="space-y-6">
                {/* Keywords */}
                <div>
                  <h2 className="text-xl font-bold text-[#1A237E] mb-4">Keyword Analysis</h2>
                ...
                {/* Section Scores */}
                <div>
                  <h2 className="text-xl font-bold text-[#1A237E] mb-4">Section Breakdown</h2>
                  <div className="space-y-4">
                    {Object.entries(atsScan.section_scores).map(([section, score]) => (
                      <div key={section}>
                        <div className="flex justify-between mb-1">
                          <span className="text-[#607086] capitalize">{section}</span>
                ...
                {/* Recommendations */}
                <div>
                  <h2 className="text-xl font-bold text-[#1A237E] mb-4">Recommendations</h2>
                  {atsScan.recommendations.length > 0 ? (
                    <ul className="space-y-3">
                      {atsScan.recommendations.map((rec, i) => (
                        <li key={i} className="flex items-start gap-3">
                          <span className="text-[#26A69A] mt-1">•</span>
                          <span className="text-[#607086]">{rec}</span>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <p className="text-[#607086]/80">Your profile looks great for this role!</p>
                  )}
                </div>
              </div>
            )}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden">
            <div className="p-4 border-b border-white/10 flex justify-between items-center">
              <h2 className="text-xl font-bold text-[#1A237E]">Resume Tailor Diff</h2>
              <button
                onClick={() => setShowDiffModal(false)}
                className="text-[#607086]/80 hover:text-[#1A237E] text-2xl"
              >
                ×
              </button>
            </div>
            <div className="p-6 overflow-auto max-h-[calc(90vh-80px)]">
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <h3 className="text-lg font-semibold text-[#607086]/80 mb-3">Original Resume</h3>
                  <div className="bg-white/40 rounded-lg p-4 text-sm">
                    <pre className="text-[#607086]/80 whitespace-pre-wrap font-sans">
                      {diffData.original}
                    </pre>
                  </div>
                </div>
                <div>
                  <h3 className="text-lg font-semibold text-[#26A69A] mb-3">Tailored Resume</h3>
                  <div className="bg-white/40 rounded-lg p-4 text-sm">
                    <pre className="text-[#607086] whitespace-pre-wrap font-sans">
                      {diffData.tailored}
                    </pre>
                  </div>
                </div>
              ...
              {/* Changes Summary */}
              <div className="mt-6">
                <h3 className="text-lg font-semibold text-[#1A237E] mb-3">Changes</h3>
                <div className="flex flex-wrap gap-2">
                  ...
                  <span className="bg-gray-500/20 text-[#607086]/80 px-3 py-1 rounded-full text-sm">
